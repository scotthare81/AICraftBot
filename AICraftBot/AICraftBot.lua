local AICraftBot_EventFrame = CreateFrame("Frame")
AICraftBot_EventFrame:RegisterEvent("PLAYER_TARGET_CHANGED")
AICraftBot_EventFrame:RegisterEvent("CHAT_MSG_WHISPER")
AICraftBot_EventFrame:RegisterEvent("CHAT_MSG_ADDON")
AICraftBot_EventFrame:RegisterEvent("CHAT_MSG_SYSTEM")
AICraftBot_EventFrame:RegisterEvent("UPDATE")
AICraftBot_EventFrame:Hide()
...
function CreateToolBar(frame, y, name, buttons, x, spacing, register)
    ...
    for key, button in pairs(buttons) do
        ...
        local filename = "Interface\\Addons\\AICraftBot\\Images\\" .. button["icon"] .. ".tga"
...
function CreateBotRoster()
    ...
    for i = 1,10 do
        ...
        cls.texture:SetTexture("Interface\\Addons\\AICraftBot\\Images\\role_dps.tga")
...
function CreateSelectedBotPanel()
    ...
    frame.header.role.texture:SetTexture("Interface/Addons/AICraftBot/Images/role_dps.tga")
...
function QueryBotParty()
    wait(0.1, function() SendBotAddonCommand("ll ?", "PARTY") end)
    wait(0.2, function() SendBotAddonCommand("formation ?", "PARTY") end)
    wait(0.3, function() SendBotAddonCommand("stance ?", "PARTY") end)
    wait(0.4, function() SendBotAddonCommand("co ?", "PARTY") end)
    wait(0.5, function() SendBotAddonCommand("nc ?", "PARTY") end)
    wait(0.6, function() SendBotAddonCommand("save mana ?", "PARTY") end)
end

function QuerySelectedBot(name)
    wait(0.1, function() SendBotAddonCommand("formation ?", "WHISPER", nil, name) end)
    wait(0.2, function() SendBotAddonCommand("stance ?", "WHISPER", nil, name) end)
    wait(0.3, function() SendBotAddonCommand("ll ?", "WHISPER", nil, name) end)
    wait(0.4, function() SendBotAddonCommand("co ?", "WHISPER", nil, name) end)
    wait(0.5, function() SendBotAddonCommand("nc ?", "WHISPER", nil, name) end)
    wait(0.6, function() SendBotAddonCommand("save mana ?", "WHISPER", nil, name) end)
    wait(0.7, function() SendBotAddonCommand("rti ?", "WHISPER", nil, name) end)
end

AICraftBot_EventFrame:SetScript("OnEvent", function(self)
    ...
    if (event == "CHAT_MSG_SYSTEM") then
        ...
        if (OnSystemMessage(message)) then
            ...
            for key,bot in pairs(botTable) do
                ...
                local filename = "Interface\\Addons\\AICraftBot\\Images\\cls_" .. string.lower(bot["class"]) ..".tga"
    ...
    if (event == "CHAT_MSG_WHISPER" or event == "CHAT_MSG_ADDON") then
        ...
        if (sender == selected) then
            ...
            local filename = "Interface\\Addons\\AICraftBot\\Images\\role_" .. bot["role"] .. ".tga"
...
SLASH_AICRAFTBOT1 = '/aibot-old'
SLASH_AICRAFTBOT2 = '/acbot'
function SlashCmdList.AICRAFTBOT(msg, editbox) -- 4.
...
print("AICraft Bot Control loaded");
